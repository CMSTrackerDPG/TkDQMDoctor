...
class TestUtilities:
    ...
    def test_get_or_create_shift_leader_group(self):
        assert not Group.objects.exists()
        group = get_or_create_shift_leader_group("Shift leaders")
    ...
    def test_update_userprofile(self):
        ...
        unsaved_user = User.objects.get()
        assert unsaved_user.userprofile.is_guest
        assert not unsaved_user.userprofile.is_shiftleader
        assert not unsaved_user.userprofile.is_expert
...
class TestCreateUserProfile(TestCase):
    ...
    def test_create_userprofile(self):
        ...
        assert userprofile == user.userprofile
        assert UserProfile.objects.exists()

        # Should not create a second UserProfile accidentally
        user = User.objects.get()
        assert userprofile == create_userprofile(user)
        assert userprofile.user == user
        assert userprofile == user.userprofile
        assert UserProfile.objects.exists()
        assert 1 == len(UserProfile.objects.all())

        # UserProfile instances should be automatically created for new Users
        user = mixer.blend(User)
