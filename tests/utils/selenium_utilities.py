...
def add_some_reference_runs(browser):
    ...
    wait_until(
        browser.find_element_by_css_selector, "tr.model-referencerun > td > a.addlink"
    ).click()
...
def select_types_and_reference_runs_in_form(browser):
    ...
    wait_until(
        Select(browser.find_element_by_id("id_reference_run")).select_by_value,
        the_value,
    )
...
def set_shift_leader_filter_date(
    browser, year_0, month_0, day_0, year_1, month_1, day_1
):
    """
    ...
    Select(browser.find_element_by_id("id_date__gte_day")).select_by_visible_text(day_0)
    ...
    Select(browser.find_element_by_id("id_date__lte_day")).select_by_visible_text(day_1)
    Select(browser.find_element_by_id("id_date__gte_year")).select_by_visible_text(
        year_1
    )
