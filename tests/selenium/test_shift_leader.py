...
class TestShiftLeader:
    ...
    def test_can_create_checklists(self, live_server, firefox, shiftleader, wait):
        ...
        firefox.find_element_by_css_selector(
            "tr.model-checklist > td > a.addlink"
        ).click()
        ...
        wait.until(EC.presence_of_element_located((By.ID, "id_checklist_general")))
    ...
    def test_delete_certification(self, live_server, firefox, shiftleader, wait):
        ...
        firefox.find_elements_by_class_name("delete_run")[1].find_element_by_tag_name(
            "a"
        ).click()
    ...
    def test_can_view_weekly_certification(
        self, live_server, firefox, shiftleader, wait
    ):
        create_recent_run()
        firefox.get("{}".format(live_server.url))
        try_to_login_user(firefox, SHIFTLEADER_USERNAME, PASSWORD)
    ...
    def test_shift_leader_report_weekly(
        self, live_server, firefox, shiftleader, wait, runs_for_slr
    ):
        firefox.get("{}".format(live_server.url))
        try_to_login_user(firefox, SHIFTLEADER_USERNAME, PASSWORD)
        wait_until(firefox.find_element_by_link_text, "Shift Leader")
    ...
    def test_shift_leader_report_day_by_day(
        self, live_server, firefox, shiftleader, wait, runs_for_slr
    ):
        firefox.get("{}".format(live_server.url))
        try_to_login_user(firefox, SHIFTLEADER_USERNAME, PASSWORD)
    ...
    def test_shift_leader_checklist(
        self, live_server, firefox, shiftleader, wait, shiftleader_checklist
    ):
        create_recent_run()
        firefox.get("{}".format(live_server.url))
        try_to_login_user(firefox, SHIFTLEADER_USERNAME, PASSWORD)
    ...
    def test_shift_leader_no_checklist(self, live_server, firefox, shiftleader, wait):
        ...
        wait_until(
            firefox.find_element_by_id, "slr-checklist"
    ...
    def test_shift_leader_popup(self, live_server, firefox, admin, wait):
        ...
        wait_until(
            firefox.find_element_by_link_text, "shiftleader__popup_enabled"
        ).click()
        wait_until(firefox.find_element_by_id, "id_raw_value").click()
        firefox.find_element_by_name("_save").click()

        wait_until(
            firefox.find_element_by_link_text, "shiftleader__popup_time_period"
        ).click()
        ...
        wait_until(firefox.find_element_by_id, "id_raw_value").clear()
        wait_until(firefox.find_element_by_id, "id_raw_value").send_keys(
            "Dont forget to do this and that."
        )
        firefox.find_element_by_name("_save").click()
